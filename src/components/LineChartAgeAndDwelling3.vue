<template>
    <div class="chart-container w-full max-w-[1500px] min-h-[600px] flex flex-col items-center justify-center px-4">
        <h3 class="text-sm text-center tracking-wide font-semibold">
            Population Percentage of HDB 5-Room And Executive Flats by Age Group Category
        </h3>
  ...
  <script>
    ...
    const grouped = d3.rollups(
      ...
      d => `${d.Year}_${ageGroupMap[d["Age Group Category"]] || d["Age Group Category"]}`
    );
    ...
    return flatData.map((row) => {
      ...
      const percentage = total === 0 ? 0 : (Number(row.Population) / Number(total)) * 100;
  ...
  watchEffect(() => {
    flatData.value = dataStore.flatAgeDist_5exec;
  ...
  watch(
    ...
    ([newFlat, newTotal]) => {
      if (Array.isArray(newFlat) && newFlat.length && Array.isArray(newTotal) && newTotal.length) {
        setDimensions();
        const percentageData = calculatePercentage(newFlat, newTotal);
        drawChart(percentageData);
      }
    },
    { immediate: true }
