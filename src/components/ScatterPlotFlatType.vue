<template>
  <div class="chart-container relative flex flex-col items-center justify-center">
    ...
    <div id="scatter-canvas-wrapper" ref="chartWrapper" class="w-full h-full relative">
...
export default {
  ...
  setup() {
    ...
    const hueOrder = ['1 ROOM', '2 ROOM', '3 ROOM', '4 ROOM', '5 ROOM', 'EXECUTIVE', 'MULTI-GENERATION'];
    const color = d3.scaleOrdinal().domain(hueOrder).range(d3.schemeTableau10);
    ...
    const drawScatterPlot = (data) => {
      ...
      const [xMean, yMean] = [d3.mean(data, d => d.FloorArea), d3.mean(data, d => d.ResalePrice)];
      const slope = d3.sum(data, d => (d.FloorArea - xMean) * (d.ResalePrice - yMean)) /
      ...
      const flatTypes = hueOrder.filter(type => data.some(d => d.FlatType === type));
      const rowHeight = 20;
      ...
      flatTypes.forEach((type, i) => {
        const centerY = legendY + legendPadding + i * rowHeight + rowHeight / 2;
