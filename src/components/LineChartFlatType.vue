    ...
    setup(props) {
        ...
        const drawLineChart = (data) => {
            ...
            const flatTypes = Array.from(
                new Set(lineData.map((d) => d.FlatType))
            ).sort();
            const years = Array.from(
                new Set(lineData.map((d) => d.Year))
            ).sort();
            ...
            const highlightRange = () => {
                ...
                if (props.shouldHighlight && props.highlightYears.length < 34) {
                    const minYear = d3.min(props.highlightYears);
                    const maxYear = d3.max(props.highlightYears);
                ...
                g.append("path")
                    ...
                    .style(
                        ...
                        props.shouldHighlight
                            ? props.highlightedFlatTypes.includes(key)
                                ? 1
                                : 0.2
                            : 1
                    );
            ...
            // if (props.shouldHighlight && props.highlightYears.length) {
            //     lineData.forEach((d) => {
            //         if (
            //             props.highlightYears.includes(d.Year) &&
            //             props.highlightedFlatTypes.includes(d.FlatType)
            //         ) {
            //             g.append("circle")
            //                 .attr("cx", x(d.Year))
            //                 .attr("cy", y(d.MedianPrice))
            //                 .attr("r", 5)
            ...
            flatTypes.forEach((type, i) => {
                ...
                legendRow.style(
                    "opacity",
                    props.shouldHighlight
                        ? props.highlightedFlatTypes.includes(type)
                            ? 1
                            : 0.3
                        : 1
                );
        ...
        onMounted(() => {
            ...
            watch(
                () => [
                    props.highlightYears,
                    props.highlightedFlatTypes,
                    props.shouldHighlight,
                ],
                () => {
                    if (latestData.value.length) {
