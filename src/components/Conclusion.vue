<template>
    <section
        ref="sectionRef"
        class="pt-40 w-full min-h-screen py-20 bg-gray-50 flex items-center justify-center"
    >
        <div
            class="w-full max-w-7xl px-6 mx-auto flex flex-col lg:flex-row gap-12 items-center justify-center"
        >
            ...
            >
                ...
                <div class="w-full max-w-lg mx-auto">
                    ...
                    >
                        <transition name="fade-fade">
                            ...
                            >
                                <h2
                                    class="text-sm sm:text-base font-bold uppercase tracking-tight mb-3"
                                >
                                    {{ currentStep.header }}
                                </h2>
                                <div
                                ...
                                >
                                    <div
                                        class="space-y-4 text-sm sm:text-base leading-relaxed text-gray-700"
                                    >
                                        <p
                                            v-for="(
                                                para, idx
                ...
                >
                    <div
                        class="sticky top-0 w-full h-screen flex flex-col items-center justify-center"
                    >
                        <!-- Legend -->
                        <transition name="fade-fade">
                            <div
                                v-if="currentStep.layout === 'split'"
                                class="mb-4 flex items-center gap-4 text-xs text-gray-600"
                            >
                                <div class="flex items-center gap-1">
                                    <span
                                        class="w-3 h-3 rounded-full bg-blue-300 border border-blue-400 transition-all"
                                    ></span>
                                    Macro Factors
                                </div>
                                <div class="flex items-center gap-1">
                                    <span
                                        class="w-3 h-3 rounded-full bg-green-300 border border-green-400 transition-all"
                                    ></span>
                                    Micro Factors
                                </div>
                            </div>
                        ...
                        <div
                            ref="factorContainerRef"
                            class="relative w-full max-w-xs mx-auto min-h-[675px]"
                        >
                            <transition name="fade-fade">
                                <div
                                    v-if="currentStep.layout === 'ranked'"
                            ...
                            <transition-group name="fade-move" tag="div">
                                <div
                                    v-for="factor in layoutFactors"
                                    :key="factor.label"
                                    :ref="
                                        (el) => factorRefs.set(factor.label, el)
                                    "
                                    ...
                                    :class="[
                                        currentStep.layout === 'ranked'
                                            ? currentStep.highlighted.includes(
                                                  factor.label
                                              )
...
const layoutFactors = computed(() => {
    return currentStep.value.layout === "split"
        ? [
              ...allFactors.filter((f) => f.scale === "macro"),
              ...allFactors.filter((f) => f.scale === "micro"),
          ]
    ...
    if (sectionEl) {
        triggers.value.push(
            ScrollTrigger.create({
                ...
                onEnter: () => (showBox.value = true),
                onEnterBack: () => (showBox.value = true),
                onLeave: () => (showBox.value = false),
                onLeaveBack: () => (showBox.value = false),
            })
        );
    }

    // Your existing step triggers (keep as is)
    steps.forEach((_, index) => {
        const el = document.querySelector(
            `[data-stepindex-trigger="${index}"]`
        );
        if (el) {
