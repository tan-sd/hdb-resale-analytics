<template>
    <div class="chart-container relative flex flex-col items-center justify-center">
      <h3 class="text-sm mb-1 text-center tracking-wide font-semibold">
        Median Income vs Resale Price (2000–2022)
      </h3>
      <div id="line-chart-income-vs-price" ref="chartContainer" class="w-full">
        <svg
          :viewBox="`0 0 ${width + margin.left + margin.right} ${height + margin.top + margin.bottom}`"
          preserveAspectRatio="xMidYMid meet"
          class="w-full h-auto min-h-[320px]"
        >
          <g
            class="chart-group"
  ...
  export default {
    ...
    methods: {
      ...
      createChart(store) {
        ...
        const incomeMap = Object.fromEntries(
          store.affordabilityIndex.map((d) => [
            ...
            d.AffordabilityIndex *
              (store.yearMedians.find((m) => m.Year === d.Year)?.median || 0),
          ])
        );
        ...
        const yLeft = d3
          .scaleLinear()
          .domain([0, d3.max(combined, (d) => d.MedianAnnualIncome) * 1.1])
          .range([this.height, 0]);
        ...
        const yRightAxis = svg.append("g").attr("transform", `translate(${this.width},0)`).call(d3.axisRight(yRight).ticks(6));
          ...
          svg.selectAll(".dot-income")
            ...
            .on("mouseover", (event, d) => {
                d3.select(tooltip)
                    ...
                    .html(
                        `Year: ${d.Year}<br>
                        <span class="tracking-wider">Income: <strong>$${new Intl.NumberFormat().format(d.MedianAnnualIncome)}</strong></span>`
                    );
        ...
        svg.selectAll(".dot-price")
            ...
            .on("mouseover", (event, d) => {
                d3.select(tooltip)
                    ...
                    .html(
                        `Year: ${d.Year}<br>
                        <span class="tracking-wider">Resale Price: <strong>$${new Intl.NumberFormat().format(d.MedianResalePrice)}</strong></span>`
                    );
                this.positionTooltip(event, tooltip);
                d3.select(event.target).attr("r", 6);
            })
