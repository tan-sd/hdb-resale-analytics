...
export default {
  ...
  setup() {
    ...
    const periods = [
      { label: "2006–2010", range: [2006, 2010], oppParties: ["SDA"] },
      { label: "2011–2012", range: [2011, 2012], oppParties: ["WP"] },
      { label: "2013–2014", range: [2013, 2014], oppParties: ["WP"] },
      { label: "2015–2019", range: [2015, 2019], oppParties: ["WP"] },
      { label: "2020–2023", range: [2020, 2023], oppParties: ["WP"] },
    ];

    for (const period of periods) {
      const df = raw.filter(d => +d.Year >= period.range[0] && +d.Year <= period.range[1]);

      const pap = df.filter(d => d["Ruling Party"] === "PAP");
      const opp = df.filter(d => period.oppParties.includes(d["Ruling Party"]));
      ...
      for (let i = 1; i < bufferFC.features.length; i++) {
        try {
          mergedBuffer = union(mergedBuffer, bufferFC.features[i]);
        } catch (e) {
      ...
      const papNearby = pap.filter(d => {
        const lat = parseFloat(d.Latitude);
        const lon = parseFloat(d.Longitude);
        return !isNaN(lat) && booleanPointInPolygon(point([lon, lat]), mergedBuffer);
      });

      const borderFlats = [
        ...opp.map(f => ({ ...f, Zone: f["Ruling Party"], period: period.label })),
        ...papNearby.map(f => ({ ...f, Zone: "PAP", period: period.label })),
      ];

      const grouped = d3.group(borderFlats, d => d.period, d => d.Zone);
      const flat = Array.from(grouped, ([period, byZone]) => {
        return Array.from(byZone, ([zone, records]) => {
          const values = records.map(d => parseFloat(d["Resale Price Adj 2024"])).filter(v => !isNaN(v));
          return { period, zone, values };
        });
      }).flat();
