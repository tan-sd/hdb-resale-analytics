<template>
  ...
  const positionTooltip = (event, tooltipElement) => {
    ...
    let top = relativeY - tooltipRect.height < 0 ? relativeY + 20 : relativeY - tooltipRect.height;
    let left =
  ...
  const drawChart = () => {
    if (!svg.value || !dataStore.chartData.length) return;
    ...
    const filteredData = dataStore.chartData.filter((d) =>
      props.selectedFlatType === "All" ? true : d["Flat Type"] === props.selectedFlatType
    );
    ...
    const x = d3
      .scaleLinear()
      .domain(d3.extent(transactionsPerYear, (d) => d.year))
